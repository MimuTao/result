...
%\begin{equation}
%\begin{bmatrix}
%\Theta_{11}&\Theta_{12}&0&\Theta_{14}&\hat{A}_{fci}&\Theta_{16}&W_{1c}M_{ai}&0\\
%*&\Theta_{22}&0&\Theta_{24}&\hat{A}_{fci}&\Theta_{26}&W_{2c}M_{ai}&0\\
%*&*&-I&L_{ci}&-\hat{L}_{fci}&0&0&0\\
%*&*&*&-P_{1aci}&-P_{2aci}&0&0&\varepsilon N^T_{ai}\\
%*&*&*&*&-P_{3aci}&0&0&0\\
%*&*&*&*&*&-\gamma^2I&0&\varepsilon N^T_{1ai}\\
%*&*&*&*&*&*&-\varepsilon I&0\\
%*&*&*&*&*&*&*&-\varepsilon I
%\end{bmatrix}
%<0
...
\begin{figure}[!htb]
\centering\includegraphics[scale=0.5]{suijiqiehuan1.eps}\\
...
\begin{figure}[!htb]
\centering\includegraphics[scale=0.5]{lianggexinhao1.eps}\\
\caption{Responses of signals}
\label{fig.2}
\end{figure}

\begin{figure}[!htb]
\centering\includegraphics[scale=0.5]{gujiwucha1.eps}\\
\caption{The filtering error}
\label{fig.2}
\end{figure}
To show the stochastic stability of the filter, we choose two different initial conditions $\xi_1(0)=[0.2 \ \  -0.2 \ \  0\ \   0]^{T}$, $\xi_2(0)=[0.5 \ \  0.5 \ \  0\ \   0]^{T}$ and  Fig.4 shows the corresponding filtering errors converge to zero when $w_k=0$. Moreover, the external disturbance $w_k$ is given as
\begin{equation}\notag
w(k)=\left\{\begin{aligned}
&0.5\mbox{rand}(1),&1\leq k\leq50\\
-&0.5\mbox{rand}(1), \ &\ \ 51< k\leq100\\
& \ \ 0,&\mbox{elsewhere}
\end{aligned}\right.
\end{equation}

Simulation results of the estimation signal and the filtering error with zero-initial conditions are depicted in Fig.5 and Fig.6, respectively, which show the good performance of the designed filter.
...
By choosing switching probabilities as $\pi_1=0.2, \pi_2=0.8$ (which is shown in Fig.7) and the expectation of the Bernoulli sequence is assumed to be $\bar{\alpha}=0.9$. Then the following filter parameters are achieved by solving LMIs in Theorem 3:
...
\end{equation}
and response curves of the estimation signal and the filtering error with zero-initial conditions are depicted in Fig.8 and Fig.9, respectively.
